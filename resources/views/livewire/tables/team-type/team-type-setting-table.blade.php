<table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-auto">
    ...
    @foreach($teamTypes as $teamType)
        <tr class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            <td class="py-4 px-5">{{ $teamType->name }}</td>
            <td class="py-4 px-5">
                {{ $teamType->teams()->count() }}
            ...
            <td class="py-4 px-5 text-right flex items-center justify-start gap-3">
                @if ($currentTab === 'archived')
                    <a wire:click="openRestoreModal({{ $teamType }})" class="cursor-pointer font-medium text-blue-600 dark:text-blue-500 hover:underline">Restore</a>
                @else
                    <a wire:click="openEditModal({{ $teamType }})" class="cursor-pointer font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                    <a wire:click="openDeleteModal({{ $teamType }})" class="cursor-pointer font-medium text-blue-600 dark:text-blue-500 hover:underline">Delete</a>
                @endif
            </td>
        </tr>
    @endforeach
