<div class="grid grid-cols-4 gap-5 mt-5">
    <div class="border border-gray-200 rounded-lg overflow-hidden h-fit">
        <ul class="divide-y divide-gray-200 w-full h-fit">
            ...
            <li wire:click="changeCategory('comment')" class="{{ $currentCategory === 'comment' ? 'active-button-group-item' : 'is-button-group-item' }}">
                <x-icon name="collection" class="w-4.5 h-4.5" />
                <span class="text-sm text-gray-700">Comment Updates</span>
            </li>
        </ul>
    </div>

    <div class="h-fit col-span-3 border border-gray-200 rounded-lg px-7 py-5 @if(count($activities) > 0) border-dashed @endif ">
            ...
            <ol class="relative border-l border-gray-200 dark:border-gray-700">
                @foreach($activities as $activity)
                    ...
                    <span class="flex absolute -left-3 justify-center items-center w-6 h-6 bg-blue-200 rounded-full ring-8 ring-white">
                        ...
                        <p class="mb-4 font-normal text-sm text-gray-500">{{ $activity->description }}</p>
        ...
        @else
            <div class="flex flex-col items-center justify-center text-center">
                ...
                <span class="w-8/12 text-sm text-gray-500 mt-2 mb-1">
                    This task dont have any activities yet. You can add activities by adding comments or attachments.
                </span>
            </div>
        @endif
    </div>
