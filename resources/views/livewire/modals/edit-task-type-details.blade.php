<x-card>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-1 gap-4 soft-scrollbar">
        ...
        <x-input label="Type Name" placeholder="Enter task type name" wire:model="name" />
    </div>
    ...
    <x-slot name="footer">
        <div class="flex justify-end gap-x-4">
            <div class="flex justify-between gap-x-2">
                ...
                @if ($actionType == 'add')
                    <x-button positive spinner="addTaskType" label="Add Task Type" wire:click="addTaskType" />
                @elseif ($actionType == 'edit')
                    <x-button positive label="Save Changes" spinner="updateTaskType" wire:click="updateTaskType" />
                @endif
            </div>
        </div>
    </x-slot>
